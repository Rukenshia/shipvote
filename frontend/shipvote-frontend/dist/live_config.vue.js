...
window.App = {
  name: 'app',
  components: {
    VoteResults: __WEBPACK_IMPORTED_MODULE_2__VoteResults__["a" /* default */]
  },
  data: function data() {
    return {
      socket: undefined,
  ...
  created: function created() {
    ...
    window.Twitch.ext.onAuthorized(function (data) {
      ...
      _this.loadChannelConfig().then(function () {
        ...
        var updateOpenVote = function updateOpenVote() {
          _this.api.getOpenVote().then(function (vote) {
            ...
            _this.stats.ship_votes = vote.votes;

            var votes = Object.values(vote.votes);

            _this.stats.votes = votes.length > 0 ? votes.reduce(function (p, v) {
              return p + v;
            }) : 0;
          }).catch(function (e) {
            return console.error('loadChannelConfig: ' + e);
          }).then(function () {
            setTimeout(function () {
              return updateOpenVote();
            }, 5000);
          });
        };
  ...
  computed: {
    ...
    bulkAddShips: function bulkAddShips() {
      var _this2 = this;

      return this.ships.filter(function (s) {
        return s.enabled;
      }).filter(function (s) {
        return !_this2.selectedShips.includes(s);
      }).filter(function (s) {
        return _this2.bulkAdd.premiums || !s.premium;
      }).filter(function (s) {
        return _this2.bulkAdd.nations === 'all' || s.nation === _this2.bulkAdd.nations;
      }).filter(function (s) {
        return _this2.bulkAdd.tiers === 'all' || s.tier === tierToInt(_this2.bulkAdd.tiers);
      }).filter(function (s) {
        return _this2.bulkAdd.types === 'all' || s.type === _this2.bulkAdd.types;
      });
    },
    searchedSelectedShips: function searchedSelectedShips() {
      var _this3 = this;

      return this.selectedShips.filter(function (s) {
        return s.name.startsWith(_this3.search);
      });
    },
    availableShips: function availableShips() {
      var _this4 = this;

      return this.ships.filter(function (s) {
        return s.enabled;
      }).filter(function (s) {
        return s.name.startsWith(_this4.search);
      }).filter(function (s) {
        return !_this4.selectedShips.includes(s);
      }).sort(function (a, b) {
        if (a.tier < b.tier) {
          return 1;
    ...
    sortedVotes: function sortedVotes() {
      ...
      var sorted = Object.keys(this.stats.ship_votes).map(function (v) {
        return _extends({}, _this5.ships.find(function (s) {
          return s.id === parseInt(v, 10);
        }), {
          votes: _this5.stats.ship_votes[v]
        });
      });

      sorted.sort(function (a, b) {
        var av = _this5.stats.ship_votes[a.id] || 0;
        var bv = _this5.stats.ship_votes[b.id] || 0;
    ...
    mostVoted: function mostVoted() {
      ...
      var sorted = Object.keys(this.stats.ship_votes).map(function (v) {
        return _extends({}, _this6.ships.find(function (s) {
          return s.id === parseInt(v, 10);
        }), {
          votes: _this6.stats.ship_votes[v]
        });
      });

      sorted.sort(function (a, b) {
        var av = _this6.stats.ship_votes[a.id] || 0;
        var bv = _this6.stats.ship_votes[b.id] || 0;
  ...
  methods: {
    ...
    loadChannelConfig: function loadChannelConfig() {
      ...
      return get(__WEBPACK_IMPORTED_MODULE_1__shipvote__["a" /* BASE_URL */] + '/api/settings/channels/' + this.channelId, {
        headers: {
          'Content-Type': 'application/json',
          authorization: 'Bearer ' + this.token
        }
      }).then(function (res) {
        _this7.ships = res.data['data']['ships'];
        _this7.configured = true;
      }).catch(function (e) {
        if (e.response.status === 404) {
          _this7.configured = false;
        } else {
          console.error(e);
          _this7.error = true;
        }
      }).then(function () {
        _this7.loaded_configuration = true;
      });
    },
    openVote: function openVote() {
      ...
      this.api.openVote(this.selectedShips.map(function (s) {
        return s.id;
      })).then(function (vote) {
        _this8.vote = vote;
        _this8.storeSelectedShips();

        if (_this8.useDuration) {
          _this8.startDurationCounter();
        }
      }).catch(function (err) {
        console.error(err);
      });
    ...
    closeVote: function closeVote() {
      ...
      this.api.closeVote(this.vote.id).then(function (vote) {
        _this9.vote = vote;
        _this9.updateClosedVotes();
      }).catch(function (err) {
        console.error(err);
      });
    },
    updateClosedVotes: function updateClosedVotes() {
      var _this10 = this;

      this.api.getClosedVotes().then(function (votes) {
        _this10.closedVotes = votes;
        _this10.loadingClosedVotes = false;
      }).catch(function (err) {
        console.error(err);
      });
    },
    startDurationCounter: function startDurationCounter() {
      var _this11 = this;
      ...
      this.elapsedInterval = setInterval(function () {
        ...
        if (_this11.elapsed >= _this11.duration * 60) {
          _this11.closeVote();
        }
      }, 1000);
    },
    doBulkAdd: function doBulkAdd() {
      var _selectedShips;

      (_selectedShips = this.selectedShips).push.apply(_selectedShips, _toConsumableArray(this.bulkAddShips));
      this.sortSelectedShips();
    },
    select: function select(ship) {
      this.selectedShips.push(ship);
      this.sortSelectedShips();
    },
    deselect: function deselect(ship) {
      var idx = this.selectedShips.findIndex(function (s) {
        return s.id === ship.id;
      });

      this.selectedShips.splice(idx, 1);
    },
    sortSelectedShips: function sortSelectedShips() {
      this.selectedShips = this.selectedShips.sort(function (a, b) {
        if (a.tier < b.tier) {
          return 1;
        } else if (a.tier == b.tier) {
          return 0;
...
/* harmony default export */ __webpack_exports__["a"] = ({
  ...
  methods: {
    topVoted: function topVoted(votes) {
      var _this = this;

      var sorted = Object.keys(votes).map(function (v) {
        return _extends({}, _this.ships.find(function (s) {
          return s.id === parseInt(v, 10);
        }), {
          votes: votes[v]
        });
    ...
    relativeTime: function relativeTime(since) {
      ...
      if (elapsed < msPerMinute) {
        var rounded = Math.round(elapsed / 1000);
        return rounded + ' second' + (rounded !== 1 ? 's' : '') + ' ago';
      } else if (elapsed < msPerHour) {
        var _rounded = Math.round(elapsed / msPerMinute);
        return _rounded + ' minute' + (_rounded !== 1 ? 's' : '') + ' ago';
      } else if (elapsed < msPerDay) {
        var _rounded2 = Math.round(elapsed / msPerHour);
        return _rounded2 + ' hour' + (_rounded2 !== 1 ? 's' : '') + ' ago';
      } else {
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_lib_template_compiler_index_id_data_v_4a532d9e_hasScoped_false_buble_transforms_node_modules_vue_loader_lib_selector_type_template_index_0_App_vue__ = __webpack_require__(55);
...
var Component = normalizeComponent(
  ...
  __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_lib_template_compiler_index_id_data_v_4a532d9e_hasScoped_false_buble_transforms_node_modules_vue_loader_lib_selector_type_template_index_0_App_vue__["a" /* default */],
...
var update = __webpack_require__(4)("fb2371ca", content, true, {});
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_lib_template_compiler_index_id_data_v_fc8e3d06_hasScoped_false_buble_transforms_node_modules_vue_loader_lib_selector_type_template_index_0_VoteResults_vue__ = __webpack_require__(54);
var normalizeComponent = __webpack_require__(2)
/* script */
...
var __vue_module_identifier__ = null
var Component = normalizeComponent(
  __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_vue_loader_lib_selector_type_script_index_0_VoteResults_vue__["a" /* default */],
  __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_lib_template_compiler_index_id_data_v_fc8e3d06_hasScoped_false_buble_transforms_node_modules_vue_loader_lib_selector_type_template_index_0_VoteResults_vue__["a" /* default */],
  __vue_template_functional__,
  __vue_styles__,
  __vue_scopeId__,
  __vue_module_identifier__
...
"use strict";
var render = function () {var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;return _c('mdc-list',{attrs:{"two-line":"","bordered":""}},_vm._l((_vm.votes.slice(_vm.votes.length - 3, _vm.votes.length).reverse()),function(vote){return _c('mdc-list-item',{key:vote.id},[_c('span',[_vm._v(_vm._s(_vm.topVoted(vote.votes)))]),_vm._v(" "),_c('span',{attrs:{"slot":"secondary"},slot:"secondary"},[_vm._v(_vm._s(_vm.relativeTime(vote.updated_at)))])])}))}
var staticRenderFns = []
