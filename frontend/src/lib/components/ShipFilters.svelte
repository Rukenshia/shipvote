<script lang="ts">
  ...
  $: {
    filteredShips = ships
      .filter((ship) =>
        shipName ? ship.name.toLowerCase().includes(shipName.toLowerCase()) : true
      )
      .filter((ship) => selectedNation === 'all' || ship.nation === selectedNation)
...
<div class="flex flex-col gap-2">
  <div class="flex items-center gap-2">
    <h2 class="text-gray-300 text-xl font-medium">Filters</h2>
    <button
      class="bg-gray-600 text-xs drop-shadow-sm hover:bg-cyan-700 transition font-medium py-0.5 px-1 rounded"
  ...
  <div class="flex flex-col gap-2">
    <div class="flex gap-2">
      <Filter name="nations" options={nations} bind:value={selectedNation}>Nations</Filter>
      <Filter name="tiers" options={tiers} bind:value={selectedTier}>Tier</Filter>
      <Filter name="types" options={types} bind:value={selectedType}>Class</Filter>
    </div>
    <div class="relative">
      <input
        type="text"
        name="ship_name"
        class="bg-gray-700 shadow-sm focus:ring-cyan-500 focus:border-cyan-500 block w-full sm:text-sm border-gray-500 rounded-md"
        placeholder="Name"
        bind:value={shipName}
      />
